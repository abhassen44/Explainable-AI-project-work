...
SAVE_DIR   = os.path.join(os.path.dirname(os.path.abspath(__file__)), '2d cnn results')
...
# Drop 5 highly-correlated features → keeps 36 features → 6×6 grid
DROP_COLS = [
...
# 1.  LOAD & PREPROCESS
# ------------------------------------------------------------------
print("1/9  Loading & preprocessing (5-class, 36 features → 6×6) …")
...
class_names   = CLASS_NAMES          # sorted alphabetically by LabelEncoder
...
# ------------------------------------------------------------------
print("2/9  Training 2D-CNN (20 epochs) to capture metrics history …")
model = build_2dcnn_model(input_shape=(6, 6, 1), num_classes=num_classes)
...
# Predictions
loss, acc = model.evaluate(X_test_2d, y_test, verbose=0)
y_pred = np.argmax(model.predict(X_test_2d, verbose=0), axis=1)
y_prob = model.predict(X_test_2d, verbose=0)
...
ax.set_title('Confusion Matrix — 2D-CNN on NSL-KDD (5-class)', fontsize=14, fontweight='bold')
plt.tight_layout()
...
ax.set_title('Classification Report — Precision / Recall / F1-Score (2D-CNN)',
...
ax.set_title('ROC-AUC Curves — One-vs-Rest (2D-CNN)', fontsize=14, fontweight='bold')
...
for i, fname in enumerate(feature_names):
    ...
    X_perm_2d = X_test_perm.reshape(-1, 6, 6, 1)
    y_perm_pred = np.argmax(model.predict(X_perm_2d, verbose=0), axis=1)
...
ax.set_title('Top 15 Feature Importance — Permutation (2D-CNN)',
             fontsize=14, fontweight='bold')
...
ax.set_title('Precision-Recall Curves — One-vs-Rest (2D-CNN)',
             fontsize=14, fontweight='bold')
...
ax.axis('off')
ax.set_title('Comprehensive Score Summary — 2D-CNN (Paper Config)',
